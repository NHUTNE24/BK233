...
const TrainingProgram = () => {
    ...
    const handleRowClick = (row) => {
        if (!editing) navigate(`${row.trainingProgramCode}`);
    };
    ...
    useEffect(() => {
        axios
            .get(`${baseUrl}/api/training-programs`, {
                headers: {
    ...
    const totalPageFilter = trainingProgramList?.filter((row) =>
        tags.every((tag) =>
            [row.name, row.days, row.createdBy, row.createdDate].some((field) =>
                field?.toString().toLowerCase().includes(tag.toLowerCase())
            )
        )
    );
    const currentData = totalPageFilter?.slice(
    ...
    const handleSubmit = (event) => {
        ...
        Papa.parse(selectedFile, {
            complete: function (results) {
                const submitedData = preprocessData(
                    results.data,
                    importSetting.scanning,
                    importSetting.duplicateHandle
                );
                const newProgram = submitedData
                    .filter(
                        (row) =>
                            row.trainingProgramCode &&
                            row.trainingProgramCode.trim() !== ''
                    )
                    .map((row) => ({
                ...
                newProgram.forEach((program) => {
                    try {
                        axios
                            .post(`${baseUrl}/api/training-programs`, program, {
                                headers: {
    ...
    const csvContent =
        'trainingProgramCode,createdBy,createdDate,modifiedBy,modifiedDate,days,hours,startTime,name,status,userId\nCC03,John,07-12-2024,John,07-12-2024,23,123,07-19-2024,Fundamental of Angular,Active,101';
    return (
        <section
            className="w-full max-h-[720px] relative bg-default"
            style={{ display: 'block' }}
        >
            <NotificationModal
            ...
            >
                ...
                <div className="flex flex-col gap-2 justify-between items-center px-10 w-full h-auto">
                    ...
                    <div className="flex flex-row w-full items-center justify-between mb-1 h-[40px]">
                        <label className="text-base text-main font-semibold">
                            File (csv)
                        </label>
                        ...
                        <div className="w-[300px] flex flex-row items-center">
                            <button
                                ...
                                className="self-start rounded-2xl text-base px-10 py-2 bg-main text-white font-bold hover:bg-secondary"
                            >
                                Select
                            </button>
                            <input
                    ...
                    <div className="flex flex-row w-full items-center justify-between mb-1 h-[40px]">
                        ...
                        <DropdownBox2
                            ...
                            options={[{ value: 'UTF-8', label: 'UTF-8' }]}
                    ...
                    <div className="flex flex-row w-full items-center justify-between mb-1 h-[40px]">
                        ...
                        <div className="w-[300px] flex flex-row items-center">
                            <div
                                ...
                                className="cursor-pointer ml-1 self-start rounded-2xl text-base text-main underline font-bold"
                            >
                                Download
                            </div>
                        </div>
                ...
                <div className="flex flex-col gap-2 justify-between items-center px-10 w-full h-auto">
                    <div className="w-full mb-2 border-b-2 border-main pb-2">
                        Duplicate control{' '}
                    </div>
                    ...
                    <div className="flex flex-row w-full items-center justify-between mb-1 h-[40px]">
                        ...
                        <div className="w-[300px] flex flex-row items-center">
                            <Radio.Group
                                ...
                                value={importSetting.scanning || null}
                            >
                    ...
                    <div className="flex flex-row w-full items-center justify-between mb-1 h-[40px]">
                        ...
                        <div className="w-[300px] flex flex-row items-center">
                            <Radio.Group
                                onChange={handleDuplicateHandle}
                                value={importSetting.duplicateHandle || null}
                            >
                ...
                <div className="w-[300px] flex flex-row gap-10 mt-1 mb-4 items-center justify-center">
                    <div
                        onClick={() => {}}
                        className="cursor-pointer text-base text-warningColor underline font-bold"
                    >
                        Cancel
                    </div>
                    <button
                        onClick={handleSubmit}
                        className="rounded-2xl text-base px-5 py-2 bg-main text-white font-bold hover:bg-secondary"
                    >
                        Import
                    </button>
                </div>
                    ...
                    <div className="w-full h-[50px] flex flex-row items-center gap-2">
                        <div className="w-full h-[50px] flex flex-row items-center gap-2">
                            ...
                            <button onClick={handleAddTag}>
                                <Button.ButtonIcon text="Filter" width="90px" />
                            </button>
                        </div>
                        <div
                            className="w-auto h-[50px] flex flex-row items-center gap-2 "
                        ...
                        <tbody className="roboto h-[auto] divide-y">
                            {currentData?.map((row, index) => (
                                <tr
                                    ...
                                    key={index}
                                    className="bg-default hover:bg-main/10  font-bold text-main text-base text-left"
                                    style={{ height: '50px !important' }}
                                >
                                    <td className="table-cell px-4 py-3 w-[10%] truncate">
                                        {(page - 1) * rowsPerPage + index + 1}
                                    </td>
                                    <td className="table-cell px-4 py-3 w-[35%] truncate">
                                        {row.name}
                                    </td>
                                    <td className="table-cell px-2 py-3 truncate">
                                        {row.createdDate?.substr(0, 10)}
                                    </td>
                                    <td className="table-cell px-4 py-3 truncate">
                                        {row.createdBy}
                                    </td>
                                    <td className="table-cell px-4 py-3 truncate">
                                        {row.days} days
