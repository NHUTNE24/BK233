...
function ItemOne() {
        ...
        rawData.map(async (item) => {
          const syllabusDetail = await Promise.all(
            item.syllabusId.map(async (item) => {
              const response = await axios.get(`${URL.LOCAL_API_SYLLABUS}/${item}`, {
                headers: {
                  'Authorization': basicAuth
                }
              })
  ...
  const handleChange = (value) => {
    const selected = trainingProgram.filter((item) => value.includes(item.trainingProgramCode));
  ...
  return (
    <div className="flex flex-col">
      ...
      {
        selectedTrainingProgram && selectedTrainingProgram.map((item, index) => (
            ...
            <AccordionSummary>
              <header key={index} className='text-primary'>
                ...
                <div className='flex flex-row gap-3 items-center'>
                  ...
                  <div>Modified on <span className='italic'>{item.modifiedOn}</span> by <span className='font-bold'>{item.modifiedBy}</span></div>
                </div>
              </header>
            </AccordionSummary>
            <AccordionDetails3>
              {
                item.syllabusDetail.map((item, index) => (
                  <div key={index} className={`flex flex-row items-center rounded-3xl overflow-hidden elevation2 ${(item.syllabus.status == "Active" ? `!text-main` : `!text-[#B9B9B9]`)}`}>
                    <img src='https://via.placeholder.com/300x96' alt='placeholder' />
                    <div className='px-5 py-4 flex flex-col gap-1'>
                      <div className='flex flex-row gap-3 items-center'>
                        <h4>{item.syllabus.topicName}</h4>
                        <Chip1 text={item.syllabus.status} closable={false} inactive={(item.syllabus.status != 'Active') ? true : false} />
                      </div>
                      <div className='flex flex-row gap-3 items-center'>
                        <p className='subtitle2 !font-normal'>{item.syllabus.topicCode + ' ' + item.syllabus.version}</p>
                        <div className={`h-5 w-0.5 ${(item.syllabus.status == "Active" ? `!bg-primary` : `!bg-inputHiddenColor`)}`}></div>
                        <p className='subtitle2 !font-normal'>{item.syllabus.days + ' days (' + item.syllabus.hours + ' hours)'}</p>
                        <div className={`h-5 w-0.5 ${(item.syllabus.status == "Active" ? `!bg-primary` : `!bg-inputHiddenColor`)}`}></div>
                        <p className='subtitle2 !font-normal'>on <span>{item.syllabus.modifiedDate}</span> by <span>{item.syllabus.modifiedBy}</span></p>
                      </div>
                    </div>
                  </div>
                )
